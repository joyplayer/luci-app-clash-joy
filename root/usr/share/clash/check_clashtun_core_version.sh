...
new_clashtun_core_version=`curl -sL ${clash_curl_proxy} "https://github.com/frainzy1477/clashtun/tags"| grep "a href=\"/frainzy1477/clashtun/releases" | head -n 1| awk -F "/tag/" '{print $2}'| sed 's/\">//'`
if [ "$?" -eq "0" ]; then
