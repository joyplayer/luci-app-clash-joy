...
if [ ! -z $check_name ];then
	...
	wget-ssl -c4 --no-check-certificate --user-agent="Clash/OpenWRT" "$clash_url" -O 2>&1 >1 $CONFIG_YAML
	if [ "$?" -eq "0" ]; then
	...
	wget-ssl -c4 --no-check-certificate --user-agent="Clash/OpenWRT" "https://gfwsb.114514.best/sub?target=clashr&url=$ssr_url" -O 2>&1 >1 $CONFIG_YAML
	if [ "$?" -eq "0" ]; then
	...
	wget-ssl -c4 --no-check-certificate --user-agent="Clash/OpenWRT" "https://tgbot.lbyczf.com/v2rayn2clash?url=$v2_url" -O 2>&1 >1 $CONFIG_YAML
	if [ "$?" -eq "0" ]; then
