...
chenckver(){
	...
	if [ $CORETYPE -eq 4 ]; then
		...
		#new_clashdtun_core_version=`wget-ssl --no-check-certificate "https://github.com/Dreamacro/clash/releases/tag/premium" | grep 'href="/Dreamacro/clash/releases/tag/premium' | head -n 1 | awk -F ' ' '{print $3}'| sed 's/<\/a>//'`
		...
		echo "  ${LOGTIME} - clash(premium) Version:${new_clashdtun_core_version}.." >> $LOG_FILE
	...
	if [ $CORETYPE -eq 3 ]; then
		...
		#new_clashtun_core_version=$(wget-ssl --no-check-certificate -t 2 -T 20 "https://github.com/frainzy1477/clashtun/tags" | grep "href=\"/frainzy1477/clashtun/releases/" | head -n 1 | awk -F "/tag/" '{print $2}' | sed 's/\">//')
		...
		echo "  ${LOGTIME} - clash(ctun) Version:${new_clashtun_core_version}.." >> $LOG_FILE
	...
	if [ $CORETYPE -eq 1 ]; then
		...
		#https://github.com/Dreamacro/clash/tags
		#new_clashr_core_version=$(wget-ssl --no-check-certificate -t 2 -T 20 "https://github.com/Dreamacro/clash/tags" | grep "href=\"/Dreamacro/clash/releases/" | head -n 1 | awk -F "/tag/" '{print $2}' | sed 's/\">//')
		new_clashr_core_version=`curl -sL ${clash_curl_proxy} "https://github.com/Dreamacro/clash/tags" | grep "/href=\"/Dreamacro/clash/releases/" | head -n 1 | awk -F "/tag/" '{print $2}' | sed 's/\">//'`
		echo "  ${LOGTIME} - clash Version:${new_clashr_core_version}.." >> $LOG_FILE
...
update() {
	...
	if [ $CORETYPE -eq 1 ]; then
	    clash_wget_dl="wget --no-check-certificate -t 2 -T 20 ${clash_wget_proxy} https://github.com/Dreamacro/clash/releases/download/"$CLASHVER"/clash-"$MODELTYPE"-"$CLASHVER".gz -O /tmp/clash.gz "
	elif [ $CORETYPE -eq 3 ]; then
		clash_wget_dl="wget --no-check-certificate -t 2 -T 20 ${clash_wget_proxy} https://github.com/frainzy1477/clashtun/releases/download/"$CLASHTUN"/clash-"$MODELTYPE".gz -O /tmp/clash.gz "
	elif [ $CORETYPE -eq 4 ]; then
		clash_wget_dl="wget --no-check-certificate -t 2 -T 20 ${clash_wget_proxy} https://github.com/Dreamacro/clash/releases/download/premium/clash-"$MODELTYPE"-"$CLASHDTUNC".gz -O /tmp/clash.gz "
	fi
	...
	if [ "$?" -eq "0" ] && [ "$(ls -l /tmp/clash.gz | awk '{print int($5)}')" -ne 0 ]; then
		...
		if [ $CORETYPE -eq 1 ]; then
			...
			rm -rf /usr/bin/clashctun >/dev/null 2>&1
			mv /tmp/clash /usr/bin/clashctun >/dev/null 2>&1
			...
			rm -rf /usr/bin/clashdtun >/dev/null 2>&1
			mv /tmp/clash /usr/bin/clashdtun >/dev/null 2>&1
			...
			tun=$(sed -n 1p /usr/share/clash/dtun_version 2>/dev/null)
			sed -i "s/${tun}/v${tun}/g" /usr/share/clash/dtun_version 2>&1
...
if [ $CORETYPE -eq 1 ] || [ $CORETYPE -eq 3 ] || [ $CORETYPE -eq 4 ]; then
	chenckver
	update
	echo "  ${LOGTIME} - Update is done!" > $LOG_FILE
	exit 0
fi
