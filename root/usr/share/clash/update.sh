...
if [ $type == "clash" ] && [ ! -z $url ];then
	...
	wget-ssl --no-check-certificate -t 2 -T 20 --user-agent="Clash/OpenWRT" $url -O 2>&1 >1 $CONFIG_YAML
...
if [ $type == "ssr2clash" ] && [ ! -z $url ];then
	...
	wget-ssl --no-check-certificate -t 2 -T 20 --user-agent="Clash/OpenWRT" "https://ssrsub2clashr.herokuapp.com/ssrsub2clash?sub=$url" -O 2>&1 >1 $CONFIG_YAML
...
if [ $type == "v2clash" ] && [ ! -z $url ];then
	...
	wget-ssl --no-check-certificate -t 2 -T 20 --user-agent="Clash/OpenWRT" "https://tgbot.lbyczf.com/v2rayn2clash?url=$url" -O 2>&1 >1 $CONFIG_YAML
