...
if [ $type == "ssr2clash" ] && [ ! -z $url ];then
	...
	wget-ssl --no-check-certificate --user-agent="Clash/OpenWRT" "https://ssrsub2clashr.herokuapp.com/ssrsub2clash?sub=$url" -O 2>&1 >1 $CONFIG_YAML
	sleep 3
	if [ "$?" -eq "0" ]; then
...
if [ $type == "v2clash" ] && [ ! -z $url ];then
	...
	wget-ssl --no-check-certificate --user-agent="Clash/OpenWRT" "https://tgbot.lbyczf.com/v2rayn2clash?url=$url" -O 2>&1 >1 $CONFIG_YAML
	sleep 3
	if [ "$?" -eq "0" ]; then
		if [ $lang == "en" ] || [ $lang == "auto" ];then
			echo "Updating Configuration Completed" >$REAL_LOG
